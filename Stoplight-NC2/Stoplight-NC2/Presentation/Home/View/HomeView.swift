    ...
    var body: some View {
        NavigationView {
            ...
            .familyActivityPicker(
                isPresented: $viewModel.isFamilyActivitySectionActive,
                selection: $tempSelection
            )
            .alert("저장 되었습니다.", isPresented: $viewModel.isSaveAlertActive) {
                Button("OK", role: .cancel) {}
            }
            .alert("모니터링 중단 시 설정한 시간과 앱이 초기화됩니다.", isPresented: $viewModel.isStopMonitoringAlertActive) {
                Button("취소", role: .cancel) {}
                Button("확인", role: .destructive) {
                    tempSelection = FamilyActivitySelection()
                    viewModel.stopScheduleMonitoring()
                }
            }
            .alert("권한 제거 시 스케쥴도 함께 제거됩니다.", isPresented: $viewModel.isRevokeAlertActive) {
                Button("취소", role: .cancel) {}
                Button("확인", role: .destructive) {
        ...
        .onAppear {
            tempSelection = viewModel.selection
        }
...
extension HomeView {
    ...
    private func savePlanButtonView() -> ToolbarItemGroup<Button<Text>> {
        ToolbarItemGroup(placement: .navigationBarTrailing) {
            ...
            Button {
                viewModel.saveSchedule(selectedApps: tempSelection)
            } label: {
                Text(BUTTON_LABEL)
            }
        }
    ...
    private func setUpTimeSectionView() -> some View {
        ...
        let times = [$viewModel.scheduleStartTime, $viewModel.scheduleEndTime]
    ...
    private func setUPAppSectionView() -> some View {
        ...
        return Section(
            header: HStack {
                ...
                Spacer()
                Button {
                    viewModel.showFamilyActivitySelection()
                } label: {
                    Text(BUTTON_LABEL)
                }
            },
    ...
    private func stopScheduleMonitoringButtonView() -> some View {
        ...
        return Button {
            viewModel.showStopMonitoringAlert()
        } label: {
            Text(BUTTON_LABEL)
                .tint(Color.red)
        }
